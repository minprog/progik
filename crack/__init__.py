...
def cracks_jason():
    ...
    check50.run("./crack 50YHuxoCN9Jkc").stdout("JH").exit(0)

@check50.check(compiles):
def cracks_malan():
    """cracks malan's password: 50QvlJWn2qJGE"""
    check50.run("./crack 50QvlJWn2qJGE").stdout("NOPE").exit(0)

@check50.check(compiles):
def cracks_mzlatkova():
    """cracks mzlatkova's password: 50CPlMDLT06yY"""
    check50.run("./crack 50CPlMDLT06yY").stdout("ha").exit(0)

@check50.check(compiles):
def cracks_patrick():
    """cracks patrick's password: 50WUNAFdX/yjA"""
    check50.run("./crack 50WUNAFdX/yjA").stdout("Yale").exit(0)

@check50.check(compiles):
def cracks_rbowden():
    """cracks rbowden's password: 50fkUxYHbnXGw"""
    check50.run("./crack 50fkUxYHbnXGw").stdout("rofl").exit(0)

@check50.check(compiles):
def cracks_summer():
    """cracks summer's password: 50C6B0oz0HWzo"""
    check50.run("./crack 50C6B0oz0HWzo").stdout("FTW").exit(0)

@check50.check(compiles):
def cracks_stelios():
    """cracks stelios's password: 50nq4RV/NVU0I"""
    check50.run("./crack 50nq4RV/NVU0I").stdout("ABC").exit(0)

@check50.check(compiles):
def cracks_wmartin():
    """cracks wmartin's password: 50vtwu4ujL.Dk"""
    check50.run("./crack 50vtwu4ujL.Dk").stdout("haha").exit(0)

@check50.check(compiles):
def cracks_zamyla():
    """cracks zamyla's password: 50i2t3sOSAZtk"""
    check50.run("./crack 50i2t3sOSAZtk").stdout("lol").exit(0)
